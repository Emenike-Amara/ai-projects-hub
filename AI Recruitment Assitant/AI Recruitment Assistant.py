...
if API_KEY == "YOUR_API_KEY_HERE" or not API_KEY:
    st.error("🔴 Error: Please replace 'YOUR_API_KEY_HERE' with your actual API key.") #exception
else:
    try:
        genai.configure(api_key=API_KEY)
        # st.success("✅ API Key configured successfully.") # Optional: can be commented out for a cleaner UI
...
with st.sidebar:
    ...
    selected_page = st.radio(
        "Services",
        ["📝 Assessment Generator", "📊 Automated CV Scorer"],
...
if selected_page == "📝 Assessment Generator":
    st.markdown("<h3>📝 Assessment Generator</h3>", unsafe_allow_html=True)
...
elif selected_page == "📊 Automated CV Scorer":
    st.markdown("<h3>📊 Automated CV Scorer</h3>", unsafe_allow_html=True)
    ...
    if st.session_state.results:
        st.success("✅ Evaluation Complete. See the ranked results below.")
        ...
        for i, result in enumerate(st.session_state.results[:num_to_display]):
            ...
            st.markdown(f"**📌 Recommendation:** {result.get('final_recommendation', 'N/A')}")
            ...
            with st.form(key=f"email_form_{i}"):
                ...
                if submitted:
                    if all([candidate_email, candidate_name, calendly_link, sender_email, sender_password]):
                        ...
                        st.warning("Please fill in all the fields within the form before sending.", icon="⚠️")
